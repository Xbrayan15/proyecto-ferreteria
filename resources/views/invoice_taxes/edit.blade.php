        ...
        <div class="col-lg-6 col-md-8">
            <div class="card shadow-lg border-0">
                <div class="card-header bg-primary text-white text-center">
                    <h1 class="h4 mb-0">Editar Impuesto de Factura</h1>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('invoice-taxes.update', $invoiceTax) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="invoice_id" id="invoice_id" class="form-select" required>
                                @foreach($invoices as $invoice)
                                    <option value="{{ $invoice->id }}" {{ $invoiceTax->invoice_id == $invoice->id ? 'selected' : '' }}>
                                        {{ $invoice->id }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="tax_id" id="tax_id" class="form-select" required>
                                @foreach($taxes as $tax)
                                    <option value="{{ $tax->id }}" {{ $invoiceTax->tax_id == $tax->id ? 'selected' : '' }}>
                                        {{ $tax->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" name="amount" id="amount" class="form-control" value="{{ $invoiceTax->amount }}" required>
                        </div>
